...
import { 
    ...
    TerminalSquare, Type, Send, ArrowLeft, Keyboard, 
    Mouse, MoveVertical, Command as CmdIcon, Trash2
} from 'lucide-react';
...
interface Props {
    command: Command; // Clean prop mapping without wrappers
}
...
export default function Controller({ command }: Props) {
    ...
    const sendPayload = () => {
        ...
        router.post(`/commands/${command.uuid}/payload`, { payload: cleanPayload }, {
            preserveScroll: true,
            onSuccess: () => {
        ...
        <div className="flex flex-col h-full flex-1 gap-6 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full">
            ...
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-4">
                    ...
                    <Heading title={`${command.name} Controller`} description={`Live remote connection to ${command.device?.name || 'Device'}`} />
                </div>
<Badge 
...
>
    ...
    {pollIntervalMs <= 1000 ? (
        ...
        <span>Idle Polling ({pollIntervalMs / 1000}s)</span>
    )}
</Badge>
            </div>

            <div ref={fullscreenRef} className={`relative group bg-black rounded-xl overflow-hidden border-2 shadow-2xl transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-[100] rounded-none w-screen h-screen' : 'aspect-video'}`}>
                {command.screenshot_path ? (
