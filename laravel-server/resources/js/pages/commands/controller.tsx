...
import { 
    MonitorPlay, RefreshCw, X, CheckCircle2, XCircle, Maximize2, Minimize2, 
    TerminalSquare, MousePointer2, Type, Send, ArrowLeft 
} from 'lucide-react';
...
export default function Controller({ command }: Props) {
    ...
    useEffect(() => {
        ...
        const timerId = setInterval(() => {
            ...
            // Silently poll database for the latest command status
            router.reload({ only: ['command'] });
        }, pollIntervalMs);
        return () => clearInterval(timerId);
    }, [pollIntervalMs, command]);
    ...
    const sendPayload = () => {
        resetPollingTimer();
        if (actions.length === 0 || !command) return;
        setSendStatus('sending');
        setSendErrorMsg('');
        const sanitizedPayload = actions.map(({ id, ...rest }) => rest);
        
        router.post(`/commands/${command.uuid}/payload`, { payload: sanitizedPayload }, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 lg:p-8 w-full max-w-7xl mx-auto">
            ...
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <Button variant="outline" size="icon" asChild>
                        <Link href={`/commands/${command.uuid}`}>
                            <ArrowLeft className="h-4 w-4" />
                        </Link>
                    </Button>
                    <Heading 
                        title={`${command.name} Controller`} 
                        description={`Live remote connection to ${command.device.name}`} 
                    />
                </div>
            ...
            {/* --- MAIN VIEWER SECTION --- */}
            <div ref={fullscreenRef} className={`relative group overflow-hidden rounded-xl border-2 shadow-2xl bg-black ${isFullscreen ? 'w-screen h-screen rounded-none border-0 flex-1' : 'aspect-video'}`}>
                ...
                {command.screenshot_path ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-muted-foreground opacity-50">
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity z-40">
                    <Badge variant="secondary" className="bg-black/50 text-white backdrop-blur-md px-3 py-1">
                        {command.device.name} {isPanMode ? '(Panning)' : '(Targeting)'}
                    </Badge>
